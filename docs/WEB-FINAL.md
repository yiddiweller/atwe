...
| # | what | where | state |
|---|---|---|---|
| T8 | **The round buttons did not match the reference.** *"Try to match this exact color and hairline, outline, color and design style and size"*, with one close-crop of a dark round ✕ | Measured off their image at 3x: a flat neutral rgb(24,24,24) fill and an rgb(57,57,57) rim. The three top-right circles were the last round buttons not on the shared control recipe, carrying a rim that composited to 29 — invisible. Matching it moved the shared token, so every button in the app is that grey now | **fixed**, build 1859, guarded by `tabrow.js` |
| T7 | **The tab row: its size, and when it appears.** *"the whole section should be better designed and spaced. It's too close to each other"*, and *"by notification page the menu options only load together with the main page"* | Apple Fitness+'s own measured geometry on all four worlds (44pt pill, 16pt padding, 10pt gap, 15pt clearance below) plus its 44pt top-right circles, the blue wash taken off every button in the app, and Notifications' pills moved out of the element the skeleton replaces so they paint with the header | **fixed**, builds 1857-1858, guarded by `tabrow.js` |
| T6 | **What reads as machine-written.** *"I wanna know if there's some other AI marks or other stuff that is noticeable that you can see it's AI, or you can see AI built the app"* | A scanner over every string a member can see, nine categories. Three acted on: the AI sparkle became Atwe's own mark, 63 emoji stopped doing icons' jobs, and 185 identical "Something went wrong" errors became traceable. Six categories deliberately left, with the reasoning written down | **fixed**, build 1856, scanned by `tools/aitells.js` |
