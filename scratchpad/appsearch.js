...
(async () => {
  ...
  // The people half of this check searches for the literal name "Zoltan". It only ever
  // passed because whoever ran it happened to have such a person in their hardcoded
  // database - the same hidden-fixture defect as the DSN, one layer up. Seed the person
  // so the probe tests what it claims instead of what the database happens to hold.
  await pool.query(
    `INSERT INTO users (name,email,password_hash,username,email_verified,onboarded)
     VALUES ('Zoltan Kovacs',$1,$2,$3,true,true)`,
    [crypto.randomUUID().slice(0, 8) + '@t.local', await auth.hashPassword('x'.repeat(12)),
     'zoltan' + crypto.randomUUID().replace(/-/g, '').slice(0, 8)]
  );
